                ...
                max_mu = 1e10)
{
  ...
  Reduce(
    f = function(pr, i) {
      ...
      while ((rcond(pr$A + ca$ln) < eps ^ 2) && (ca$mu < max_mu))
      {
        ca$mu = min(ca$mu * ca$nu, max_mu)
        ca$nu = min(2 * ca$nu, 2 ^ 5)
        ca$ln = ca$mu * diag(diag(pr$A))
      }
      if ((rcond(pr$A + ca$ln) < eps ^ 2))
      {
        ca$ln = ca$mu * max(diag(pr$A),1) * diag(nrow(pr$A))
      }
      ...
      Fn = f(ca$x, xdata[ii,])
      yn = ydata[ii,]
      ca$sqrsum_i = sum((Fn - yn) ^ 2)
      Jn = Jacobian(ca$x, xdata[ii,])
      ca$A = t(Jn) %*% Jn
      ca$gradient_i = t(Jn) %*% (Fn - yn)
